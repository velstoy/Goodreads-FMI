...
std::shared_ptr<User> UserFactory::createUser(const std::string& type, const std::string& username,
	const std::string& password, const Date& registrationDate, const std::optional<Date>& birthday)
{
	if (type == "reader")
		return std::make_shared<Reader>(username, password, registrationDate, birthday);
	if (type == "author")
		return std::make_shared<Author>(username, password, registrationDate, birthday);
	if (type == "publisher")
